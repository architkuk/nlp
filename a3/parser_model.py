...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        weight = nn.Parameter(torch.empty(n_features * self.embed_size, hidden_size))
        bias = nn.Parameter(torch.empty(hidden_size))
        self.embed_to_hidden_weight = nn.init.xavier_uniform_(weight)
        self.embed_to_hidden_bias = nn.init.uniform_(bias)
        self.dropout = nn.Dropout(p=dropout_prob)
        l_weight = nn.Parameter(torch.empty(hidden_size, n_classes))
        l_bias = nn.Parameter(torch.empty(n_classes))
        self.hidden_to_logits_weight = nn.init.xavier_uniform_(l_weight)
        self.hidden_to_logits_bias = nn.init.uniform_(l_bias)
        ### END YOUR CODE
    ...
    def forward(self, w):
        ...
        h = F.relu(self.embedding_lookup(w) @ self.embed_to_hidden_weight + self.embed_to_hidden_bias)
