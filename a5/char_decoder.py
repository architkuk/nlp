...
class CharDecoder(nn.Module):
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        input = char_sequence[0:len(char_sequence) - 1]
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        ## INCOMPLETE
        batch_size = initialStates[1]
        start = self.target_vocab.start_of_word
        end = self.target_vocab.end_of_word
        input = torch.tensor([start] * max_length, device = device).unsqueeze(0)
        output_words = []
        dec_hidden = None
        for i in range(max_length):
            h, dec_hidden = self.forward(input, dec_hidden)
            p = F.softmax(h)
            current_char = torch.argmax(p)
            if current_char == '}':
                break
